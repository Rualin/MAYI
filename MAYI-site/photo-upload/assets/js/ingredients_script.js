...
document.getElementById('recipeBtn').addEventListener('click', async function() {
    ...
    try {
        ...
        const serverUrl = 'http://127.0.0.1:8000/submit';
        ...
        // Отображаем реальные рецепты из ответа сервера
        displayRecipes(data);
...
function displayRecipes(response) {
    ...
    if (!response?.success || !response.dishes?.length) {
        recipesContainer.innerHTML = `
    ...
    // Ограничиваем количество отображаемых рецептов (например, 3)
    const recipesToShow = response.dishes.slice(0, 3);
    ...
    recipesToShow.forEach(recipe => {
        ...
        // Заполняем карточку рецепта реальными данными
        card.innerHTML = `
